...
namespace SO03
{
    public partial class FormRAM : Form
    {
        ...
        FormMain currentForm;
        Random random = new Random();
        bool done = false;
        bool paused = false;
        private int physicalSize;

        public FormMain CurrentForm
        {
            get
            {
                return currentForm;
            ...
            {
                currentForm = value;
                Driver.GenerateRequests(currentForm, out RequestList);
            }
        }
        ...
        public FormRAM(int physSize, int virtSize, List<int> reqStream, algorithm alg = algorithm.LRU) : this()
        {
        ...
        private void TickLRU()
        {
            ...
            if (currentRequest == -1)
            {
                endOfStream(this, null); // propably reload will happen
                Pause();
                //GetCurrentRequest(out currentRequest);
            }
            if (RAM.Find(x => x.number == currentRequest) == null) //request not in RAM
            {
                ...
                {
                    List<int> streamDoneReversed = new List<int>(currentForm.requestStreamDone);
        ...
        private void GetCurrentRequest(out Request currentRequest)
        {
            ...
            currentForm.requestStreamDone.Add(currentRequestNumber);
